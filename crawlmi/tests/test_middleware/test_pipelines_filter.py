...
class FilterUrlLengthTest(unittest.TestCase):
    def test_nontext_response(self):
        ...
        mw = Filter(get_engine(FILTER_NONTEXT_RESPONSE=False))
        bad3 = mw.process_response(bad1)
        self.assertIs(bad1, bad3)

    def test_url_length_limit(self):
        mw = Filter(get_engine(FILTER_URL_LENGTH_LIMIT=11))
    ...
    def test_filter_non_200(self):
        mw = Filter(get_engine(FILTER_NON_200_RESPONSE_STATUS=True))
        req = Request('http://github.com/')
        ...
        mw = Filter(get_engine(FILTER_NON_200_RESPONSE_STATUS=False))
        bad3 = mw.process_response(bad1)
        self.assertIs(bad1, bad3)

    def test_response_status(self):
        mw = Filter(get_engine(FILTER_RESPONSE_STATUS=lambda x: x != 201))
        req = Request('http://github.com/')
